...
public class LoginController {
    ...
    public ResultVo sendregcode(HttpServletRequest request) throws IOException{
        ...
        if(result == 1){
            phonecodemap1.put(mobilephone, code);
    ...
    public  ResultVo userReg(@RequestBody UserInfo userInfo, HttpSession session) {
        ...
        if (!StringUtils.isEmpty(userNameIsExist)){
        ...
        if (rel.equalsIgnoreCase(vercode)) {
            ...
            userInfo.setUserid(userid).setPassword(passwords).setUimage("/pic/d1d66c3ea71044a9b938b00859ca94df.jpg").
                    setSign("欢迎来到智能化共享租赁平台").setStatus("offline");
            ...
            if (integer==1 && integer1==1){
                ...
                UsernamePasswordToken token=new UsernamePasswordToken(mobilephone, new Md5Hash(password,"Lease-Shop").toString());
    ...
    public ResultVo userLogin(@RequestBody Login login, HttpSession session){
        ...
        if (!JustPhone.justPhone(account)) {
            ...
            token=new UsernamePasswordToken(mobilephone, new Md5Hash(password,"Lease-Shop").toString());
        }
        Subject subject= SecurityUtils.getSubject();
        try {
            subject.login(token);
            ...
            login.setRoleid(loginService.findRoleId(login1.getUserid()));
            session.setAttribute("roleid",login1.getRoleid());
    ...
    public ResultVo sendresetpwd(HttpServletRequest request) throws IOException {
        ...
        Integer result = new SmsUtil().SendMsg(mobilephone, code, type);
        if(result == 1) {
            phonecodemap2.put(mobilephone, code);
            //执行定时任务
            ScheduledExecutorService executorService = new ScheduledThreadPoolExecutor(1,
                    new BasicThreadFactory.Builder().namingPattern("example-schedule-pool-%d").daemon(true).build());
            executorService.scheduleAtFixedRate(new Runnable() {
